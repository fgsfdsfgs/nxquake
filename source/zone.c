...
void Z_Free(const void *ptr) {
    ...
    if (!ptr) Sys_Error("%s: NULL pointer", __func__);

    block = (memblock_t *)((const byte *)ptr - sizeof(memblock_t));
    if (block->id != ZONEID) Sys_Error("%s: freed a pointer without ZONEID", __func__);
    if (block->tag == 0) Sys_Error("%s: freed a freed pointer", __func__);
...
static void *Z_TagMalloc(int size, int tag) {
    ...
    if (!tag) Sys_Error("%s: tried to use a 0 tag", __func__);
...
void *Z_Malloc(int size) {
    ...
    if (!buf) Sys_Error("%s: failed on allocation of %i bytes", __func__, size);
    memset(buf, 0, size);
 ...
 * ========================
 */
static void Z_Print(const memzone_t *zone, qboolean detailed) {
...
void Hunk_Check(void) {
    ...
    while ((byte *)hunk != hunkstate.base + hunkstate.lowbytes) {
        if (hunk->sentinal != HUNK_SENTINAL) Sys_Error("%s: trashed sentinal", __func__);
...
static void Hunk_Print(qboolean all) {
    ...
    while (1) {
         ...
         */
        if (hunk == endhigh) break;

        /*
         * run consistancy checks
         */
        if (hunk->sentinal != HUNK_SENTINAL) Sys_Error("%s: trashed sentinal", __func__);
        if (hunk->size < 16) Sys_Error("%s: bad size", __func__);
        ...
        if (all) Con_Printf("%*p :%10i %-*s\n", pwidth, hunk, hunk->size, HUNK_NAMELEN, safename);
        ...
        if (next != endlow && next != endhigh)
            if (!strncmp(hunk->name, next->name, HUNK_NAMELEN)) continue;
        /*
         * print the total
         */
        if (!all) Con_Printf("%*s :%10i %-*s (TOTAL)\n", pwidth, "", sum, HUNK_NAMELEN, safename);
        count = 0;
        sum = 0;
    }
    Con_Printf("-------------------------\n");
...
void *Hunk_AllocName(int size, const char *name) {
    ...
    if (size < 0) Sys_Error("%s: bad size: %i", __func__, size);
...
void *Hunk_AllocExtend(const void *base, int size) {
    ...
    if (base != hunkstate.lowbase) Sys_Error("%s: base != current base", __func__);
...
int Hunk_LowMark(void) { return hunkstate.lowbytes; }

void Hunk_FreeToLowMark(int mark) {
    if (mark < 0 || mark > hunkstate.lowbytes) Sys_Error("%s: bad mark %i", __func__, mark);
...
void Hunk_FreeToHighMark(int mark) {
    ...
    if (mark < 0 || mark > hunkstate.highbytes) Sys_Error("%s: bad mark %i", __func__, mark);
...
void *Hunk_TempAllocExtend(int size) {
    ...
    if (!hunkstate.tempmark) Sys_Error("%s: temp hunk not active", __func__);

    old = (hunk_t *)(hunkstate.base + hunkstate.size - hunkstate.highbytes);
    if (old->sentinal != HUNK_SENTINAL) Sys_Error("%s: old sentinal trashed\n", __func__);
...
static inline void *Cache_Data(const cache_system_t *c) { return (byte *)(c + 1) + c->user->pad; }
...
void Cache_CheckLinks(void) {
    ...
    while (cache != &cache_head) cache = cache->next;

    cache = cache_head.prev;
    while (cache != &cache_head) cache = cache->prev;

    cache = cache_head.lru_next;
    while (cache != &cache_head) cache = cache->lru_next;

    cache = cache_head.lru_prev;
    while (cache != &cache_head) cache = cache->lru_prev;
}
#endif
 ...
 * ===========
 */
static void Cache_Move(cache_system_t *old_cs) {
...
static void Cache_UnlinkLRU(cache_system_t *cs) {
    if (!cs->lru_next || !cs->lru_prev) Sys_Error("%s: NULL link", __func__);
...
static void Cache_MakeLRU(cache_system_t *cs) {
    if (cs->lru_next || cs->lru_prev) Sys_Error("%s: active link", __func__);
...
void Cache_Report(void) {
    Con_DPrintf("%4.1f megabyte data cache\n",
                (hunkstate.size - hunkstate.highbytes - hunkstate.lowbytes) / (float)(1024 * 1024));
}
...
static void Cache_Dealloc(cache_user_t *c) {
    ...
    if (!c->data) Sys_Error("%s: not allocated", __func__);
...
void *Cache_AllocPadded(cache_user_t *c, int pad, int size, const char *name) {
    ...
    if (c->data) Sys_Error("%s: already allocated", __func__);

    if (size <= 0) Sys_Error("%s: size %i", __func__, size);
    ...
    while (1) {
        ...
        if (cache_head.lru_prev == &cache_head) Sys_Error("%s: out of memory", __func__);
...
size_t Memory_GetSize(void) {
    ...
    if (parm && com_argc > parm + 1) return (size_t)atoi(com_argv[parm + 1]) << 20;

    /* The -heapsize parameter gives the heap size in kilobytes */
    parm = COM_CheckParm("-heapsize");
    if (parm && com_argc > parm + 1) return (size_t)atoi(com_argv[parm + 1]) << 10;
