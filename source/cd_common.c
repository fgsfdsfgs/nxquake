...
cvar_t bgmvolume = {
    .name = "bgmvolume", .string = "1", .archive = true, .callback = CDAudio_SetVolume_f};

static void CDAudio_Eject(void) {
    if (enabled) CDDrv_Eject();
}

static void CDAudio_CloseDoor(void) {
    if (enabled) CDDrv_CloseDoor();
}
...
static void CDAudio_SetVolume_f(struct cvar_s *var) {
    ...
    if (cdvolume != var->value) {
        ...
        if (ret >= 0) cdvolume = (float)ret / 255.0;
     ...
     */
    if (changed) Cvar_SetValue("bgmvolume", var->value);
}
...
void CDAudio_InvalidateDisk(void) { cdValid = false; }
...
static void CD_f(void) {
    ...
    if (strcasecmp(command, "remap") == 0) {
        ...
        for (n = 1; n <= ret; n++) remap[n] = Q_atoi(Cmd_Argv(n + 1));
        return;
    }
