...
static void R_AliasSetUpTransform(const entity_t *e, aliashdr_t *pahdr, int trivial_accept);
...
static void SW_LoadSkinData(model_t *model, aliashdr_t *ahdr, const alias_skindata_t *skindata) {
    int i, j, skinsize;
    byte *pixels;
    ...
    for (i = 0; i < skindata->numskins; i++) {
        if (r_pixbytes == 1) {
            ...
            for (j = 0; j < skinsize; j++) skin16[j] = d_8to16table[skindata->data[i][j]];
        } else {
            Sys_Error("%s: driver set invalid r_pixbytes: %d", __func__, r_pixbytes);
        }
        pixels += skinsize * r_pixbytes;
    }
}

static void SW_LoadMeshData(const model_t *model, aliashdr_t *hdr, const alias_meshdata_t *meshdata,
...
const model_loader_t *R_ModelLoader(void) { return &SW_Model_Loader; }
...
qboolean R_AliasCheckBBox(entity_t *e) {
    ...
    for (i = 0; i < 8; i++) {
        ...
        if (viewaux[i].fv[2] < ALIAS_Z_CLIP_PLANE) {
            ...
            if (viewaux[i].fv[2] < minz) minz = viewaux[i].fv[2];
            viewpts[i].flags = 0;
            zfullyclipped = false;
        }
    }
    ...
    for (i = 0; i < numv; i++) {
        ...
        if (viewpts[i].flags & ALIAS_Z_CLIP) continue;
     ...
     */
    if (r_lerpmodels.value) return true;
#endif
...
================
*/
static void R_AliasPreparePoints(aliashdr_t *pahdr, finalvert_t *pfinalverts,
                                 auxvert_t *pauxverts) {
    int i;
    stvert_t *pstverts;
    finalvert_t *fv;
...
void R_AliasTransformAndProjectFinalVerts(finalvert_t *fv, stvert_t *pstverts) {
    ...
    for (i = 0; i < r_anumverts; i++, fv++, pverts++, pstverts++) {
        ...
        fv->v[0] =
            ((DotProduct(pverts->v, aliastransform[0]) + aliastransform[0][3]) * zi) + aliasxcenter;
        fv->v[1] =
            ((DotProduct(pverts->v, aliastransform[1]) + aliastransform[1][3]) * zi) + aliasycenter;
...
static void R_AliasPrepareUnclippedPoints(aliashdr_t *pahdr, finalvert_t *pfinalverts) {
    ...
    if (r_affinetridesc.drawtype) D_PolysetDrawFinalVerts(pfinalverts, r_anumverts);
...
static void R_AliasSetupSkin(const entity_t *entity, aliashdr_t *aliashdr) {
    ...
    if (entity->scoreboard) {
        ...
        if (!entity->scoreboard->skin) Skin_Find(entity->scoreboard);
...
#ifdef NQ_HACK
static trivertx_t *R_AliasBlendPoseVerts(const entity_t *e, aliashdr_t *hdr, float blend) {
...
=================
*/
static void R_AliasSetupFrame(entity_t *e, aliashdr_t *pahdr) {
    ...
    if (r_lerpmodels.value) {
        ...
        if (e->currentframetime < e->previousframetime) goto nolerp;
        if (e->currentframetime - e->previousframetime > 1.0f) goto nolerp;
        /* FIXME - hack to skip the viewent (weapon) */
        if (e == &cl.viewent) goto nolerp;
...
void R_AliasDrawModel(entity_t *e, alight_t *plighting) {
    ...
    if (!e->colormap) Sys_Error("%s: !e->colormap", __func__);

    r_affinetridesc.drawtype = (e->trivial_accept == 3) && r_recursiveaffinetriangles;
