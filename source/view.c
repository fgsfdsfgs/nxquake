...
float V_CalcBob(void) {
    ...
    if (!cl_bobcycle.value) return 0.0f;
...
void V_UpdatePalette(void) {
    ...
    if (force) {
        ...
        if (VID_IsFullScreen() && VID_SetGammaRamp) VID_SetGammaRamp(ramps);
...
void V_CalcRefdef(void) {
    ...
    for (i = 0; i < 3; i++)
        r_refdef.vieworg[i] +=
            scr_ofsx.value * forward[i] + scr_ofsy.value * right[i] + scr_ofsz.value * up[i];
...
#ifndef GLQUAKE
    if (crosshair.value) Draw_Crosshair();
#endif
}
