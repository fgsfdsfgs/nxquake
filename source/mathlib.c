...
==================
*/
void __attribute__((noreturn)) BOPS_Error(void) { Sys_Error("%s:  Bad signbits", __func__); }
...
int BoxOnPlaneSide(const vec3_t mins, const vec3_t maxs, const mplane_t *p) {
    ...
    sides = 0;
    if (dist1 >= p->dist) sides = PSIDE_FRONT;
    if (dist2 < p->dist) sides |= PSIDE_BACK;

#ifdef PARANOID
    if (sides == 0) Sys_Error("%s: sides == 0", __func__);
#endif
...
#ifndef PARANOID
    if (denom <= 0.0) Sys_Error("%s: bad denominator %lf", __func__, denom);
