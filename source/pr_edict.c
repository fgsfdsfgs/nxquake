...
#ifdef NQ_HACK
        if (i == MAX_EDICTS) SV_Error("%s: no free edicts", __func__);
        sv.num_edicts++;
#endif
#if defined(QW_HACK) && defined(SERVERONLY)
        if (i == MAX_EDICTS) {
    ...
    static char *PR_ValueString(etype_t type, eval_t * val) {
        ...
        switch (type) {
            ...
            case ev_float:
                ...
                break;
            case ev_vector:
                snprintf(line, sizeof(line), "'%5.1f %5.1f %5.1f'", val->vector[0], val->vector[1],
                         val->vector[2]);
                break;
            case ev_pointer:
    ...
    static char *PR_UglyValueString(etype_t type, eval_t * val) {
        ...
        switch (type) {
            ...
            case ev_float:
                ...
                break;
            case ev_vector:
                snprintf(line, sizeof(line), "%f %f %f", val->vector[0], val->vector[1],
                         val->vector[2]);
                break;
            default:
                snprintf(line, sizeof(line), "bad type %i", type);
...
*/
    char *PR_GlobalString(int ofs) {
        ...
        for (i = strlen(line); i < 20; i++) strcat(line, " ");
        strcat(line, " ");
...
*/
    void ED_ParseGlobals(const char *data) {
        ...
        while (1) {
            ...
            if (!data) SV_Error("%s: EOF without closing brace", __func__);
            ...
            if (com_token[0] == '}') SV_Error("%s: closing brace without data", __func__);
...
*/
    const char *ED_ParseEdict(const char *data, edict_t *ent) {
        ...
        while (1) {
            ...
            n = strlen(keyname);
            while (n && keyname[n - 1] == ' ') keyname[--n] = 0;
...
*/
    void ED_LoadFromFile(const char *data) {
        ...
        while (1) {
            ...
            data = COM_Parse(data);
            if (!data) break;
            if (com_token[0] != '{') SV_Error("%s: found %s when expecting {", __func__, com_token);
...
*/
    void PR_LoadProgs(void) {
        ...
        if (!progs) progs = COM_LoadHunkFile("progs.dat");
#endif
        if (!progs) SV_Error("%s: couldn't load progs.dat", __func__);
        ...
        if (progs->version != PROG_VERSION)
            SV_Error("progs.dat has wrong version number (%i should be %i)", progs->version,
                     PROG_VERSION);
...
*/
    ...
    int NUM_FOR_EDICT(const edict_t *e) {
        ...
        if (b < 0 || b >= sv.num_edicts) SV_Error("%s: bad pointer", __func__);
        return b;
    }
