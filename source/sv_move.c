    ...
    if ((int)entity->v.flags & (FL_SWIM | FL_FLY)) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            if (i == 0 && enemy != sv.edicts) {
                dz = entity->v.origin[2] - PROG_TO_EDICT(entity->v.enemy)->v.origin[2];
                if (dz > 40) neworg[2] -= 8;
                if (dz < 30) neworg[2] += 8;
            }
            SV_TraceMoveEntity(entity, entity->v.origin, neworg, MOVE_NORMAL, &trace);
            ...
            if (trace.fraction == 1) {
                ...
                if (relink) SV_LinkEdict(entity, true);
                return true;
            }
