...
void VID_Shutdown(void) {
    ...
    if (sdl_format) SDL_FreeFormat(sdl_format);
    if (sdl_desktop_format) SDL_FreeFormat(sdl_desktop_format);
}
...
static cvar_t _vid_default_mode_win = {.name = "_vid_default_mode_win",
                                       ...
                                       .archive = true};
static cvar_t vid_fullscreen_mode = {.name = "vid_fullscreen_mode",
                                     .string = stringify(VID_MODE_FULLSCREEN_DEFAULT),
                                     .archive = true};
static cvar_t vid_windowed_mode = {
    .name = "vid_windowed_mode", .string = stringify(VID_MODE_WINDOWED), .archive = true};
...
qboolean VID_CheckAdequateMem(int width, int height) {
     ...
     */
    if ((host_parms.memsize - tbuffersize + SURFCACHE_SIZE_AT_320X200 + 0x10000 * 3) <
        minimum_memory)
        return false;
...
qboolean VID_IsFullScreen() { return !!vid_modenum; }
...
qboolean VID_SetMode(const qvidmode_t *mode, const byte *palette) {
    ...
    sdl_window = SDL_CreateWindow("TyrQuake", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
                                  mode->width, mode->height, flags);
    if (!sdl_window) Sys_Error("%s: Unable to create window: %s", __func__, SDL_GetError());

    renderer = SDL_CreateRenderer(sdl_window, -1, 0);
    if (!renderer) Sys_Error("%s: Unable to create renderer: %s", __func__, SDL_GetError());

    texture = SDL_CreateTexture(renderer, format->format, SDL_TEXTUREACCESS_STREAMING, mode->width,
                                mode->height);
    if (!texture) Sys_Error("%s: Unable to create texture: %s", __func__, SDL_GetError());
...
void VID_Init(const byte *palette) {
    ...
    if (err < 0) Sys_Error("VID: Couldn't load SDL: %s", SDL_GetError());

    err = SDL_GetDesktopDisplayMode(0, &desktop_mode);
    if (err) Sys_Error("%s: Unable to query desktop display mode (%s)", __func__, SDL_GetError());
    sdl_desktop_format = SDL_AllocFormat(desktop_mode.format);
    if (!sdl_desktop_format)
        Sys_Error("%s: Unable to allocate desktop pixel format (%s)", __func__, SDL_GetError());
...
void VID_Update(vrect_t *rects) {
    ...
    for (rect = rects; rect; rect = rect->pnext) {
        ...
        if (err) Sys_Error("%s: unable to lock texture (%s)", __func__, SDL_GetError());
        ...
        switch (SDL_PIXELTYPE(sdl_format->format)) {
            case SDL_PIXELTYPE_PACKED32:
                dst32 = dst;
                while (height--) {
                    for (i = 0; i < rect->width; i++) dst32[i] = d_8to24table[src[i]];
            ...
            case SDL_PIXELTYPE_PACKED16:
                dst16 = dst;
                while (height--) {
                    for (i = 0; i < rect->width; i++) dst16[i] = d_8to16table[src[i]];
    ...
    if (err) Sys_Error("%s: unable to render texture (%s)", __func__, SDL_GetError());
...
void D_BeginDirectRect(int x, int y, const byte *pbitmap, int width, int height) {
    ...
    src = pbitmap;
    while (height--) {
        for (i = 0; i < width; i++) dst[i] = d_8to24table[src[i]];
