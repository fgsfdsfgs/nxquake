...
void R_Alias_clip_left(finalvert_t *pfv0, finalvert_t *pfv1, finalvert_t *out) {
    ...
    if (pfv0->v[1] >= pfv1->v[1]) {
        ...
        for (i = 0; i < 6; i++) out->v[i] = pfv0->v[i] + (pfv1->v[i] - pfv0->v[i]) * scale + 0.5;
    } else {
        scale = (float)(r_refdef.aliasvrect.x - pfv1->v[0]) / (pfv0->v[0] - pfv1->v[0]);
        for (i = 0; i < 6; i++) out->v[i] = pfv1->v[i] + (pfv0->v[i] - pfv1->v[i]) * scale + 0.5;
    }
}
...
void R_AliasClipTriangle(mtriangle_t *ptri, finalvert_t *pfinalverts, auxvert_t *pauxverts) {
    ...
    if (clipflags & ALIAS_Z_CLIP) {
        for (i = 0; i < 3; i++) av[i] = pauxverts[ptri->vertindex[i]];
