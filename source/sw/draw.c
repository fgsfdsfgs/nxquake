...
================
*/
static cachepic_t *Draw_FindCache(const char *path) {
    cachepic_t *cachepic;
    int i;

    cachepic = menu_cachepics;
    for (i = 0; i < menu_numcachepics; i++, cachepic++)
        if (!strcmp(path, cachepic->name)) break;

    if (i == menu_numcachepics) {
        if (menu_numcachepics == MAX_CACHED_PICS) Sys_Error("menu_numcachepics == MAX_CACHED_PICS");
        menu_numcachepics++;
...
const qpic8_t *Draw_CachePic(const char *path) {
    ...
    dpic = cachepic->cache.data;
    if (!dpic) Sys_Error("%s: failed to load %s", __func__, path);
    SwapDPic(dpic);
...
const qpic8_t *Draw_CacheConback(void) {
    ...
    dpic = cachepic->cache.data;
    if (!dpic) Sys_Error("%s: failed to load %s", __func__, conbackfile);
    SwapDPic(dpic);
...
void Draw_Character(int x, int y, int num) {
    ...
    if (r_pixbytes == 1) {
        ...
        pusdest = (uint16_t *)((byte *)vid.conbuffer + y * vid.conrowbytes + (x << 1));

        while (drawline--) {
            if (source[0]) pusdest[0] = d_8to16table[source[0]];
            if (source[1]) pusdest[1] = d_8to16table[source[1]];
            if (source[2]) pusdest[2] = d_8to16table[source[2]];
            if (source[3]) pusdest[3] = d_8to16table[source[3]];
            if (source[4]) pusdest[4] = d_8to16table[source[4]];
            if (source[5]) pusdest[5] = d_8to16table[source[5]];
            if (source[6]) pusdest[6] = d_8to16table[source[6]];
            if (source[7]) pusdest[7] = d_8to16table[source[7]];
...
void Draw_TransPic(int x, int y, const qpic8_t *pic) {
    ...
    if (r_pixbytes == 1) {
        ...
        if (pic->width & 7) {  // general
            for (v = 0; v < pic->height; v++) {
                for (u = 0; u < pic->width; u++)
                    if ((tbyte = source[u]) != TRANSPARENT_COLOR) dest[u] = tbyte;
            ...
            for (v = 0; v < pic->height; v++) {
                for (u = 0; u < pic->width; u += 8) {
                    if ((tbyte = source[u]) != TRANSPARENT_COLOR) dest[u] = tbyte;
                    if ((tbyte = source[u + 1]) != TRANSPARENT_COLOR) dest[u + 1] = tbyte;
                    if ((tbyte = source[u + 2]) != TRANSPARENT_COLOR) dest[u + 2] = tbyte;
                    if ((tbyte = source[u + 3]) != TRANSPARENT_COLOR) dest[u + 3] = tbyte;
                    if ((tbyte = source[u + 4]) != TRANSPARENT_COLOR) dest[u + 4] = tbyte;
                    if ((tbyte = source[u + 5]) != TRANSPARENT_COLOR) dest[u + 5] = tbyte;
                    if ((tbyte = source[u + 6]) != TRANSPARENT_COLOR) dest[u + 6] = tbyte;
                    if ((tbyte = source[u + 7]) != TRANSPARENT_COLOR) dest[u + 7] = tbyte;
                }
                dest += vid.rowbytes;
                source += pic->width;
            }
...
static void Draw_ScaledCharToConback(const qpic8_t *conback, int num, byte *dest) {
    ...
    for (y = 0; y < drawlines; y++, dest += conback->width) {
        ...
        for (x = 0; x < drawwidth; x++, f += fstep) {
            if (src[f >> 16]) dest[x] = 0x60 + src[f >> 16];
        }
    }
}
...
==============
*/
static void R_DrawRect16(vrect_t *prect, int rowbytes, const byte *psrc, int transparent) {
    byte t;
    int i, j, srcdelta, destdelta;
    uint16_t *pdest;
...
void Draw_TileClear(int x, int y, int w, int h) {
    ...
    if (x < 0 || (unsigned)(x + w) > vid.width || y < 0 || (unsigned)(y + h) > vid.height) {
        Sys_Error("%s: bad coordinates", __func__);
    }
