...
void R_InsertNewEdges(edge_t *edgestoadd, edge_t *edgelist) {
    ...
    edgesearch:
        if (edgelist->u >= edgestoadd->u) goto addedge;
...
void R_TrailingEdge(surf_t *surf, edge_t *edge) {
    ...
    if (surf->insubmodel) r_bmodelactive--;
...
void R_LeadingEdge(edge_t *edge) {
    ...
    if (surf->insubmodel && (surf->key == surf2->key)) {
        ...
        newzitop = newzi * 1.01;
        if (newzitop >= testzi && surf->d_zistepu >= surf2->d_zistepu) goto newtop;
    }
...
continue_search:
    ...
    if (surf->key != surf2->key) goto gotposition;
    ...
    newzitop = newzi * 1.01;
    if (newzitop >= testzi && surf->d_zistepu >= surf2->d_zistepu) goto gotposition;
