...
void R_Init(void) {
    ...
    view_clipplanes[0].rightedge = view_clipplanes[2].rightedge = view_clipplanes[3].rightedge =
        false;
...
void R_NewMap(void) {
    ...
    for (i = 0; i < cl.worldmodel->numleafs; i++) cl.worldmodel->leafs[i].efrags = NULL;
...
===============
*/
static void R_MarkSurfaces(void) {
...
=============
*/
static void R_CullSurfaces(brushmodel_t *brushmodel, vec3_t vieworg) {
    int i, j;
    int side;
    mnode_t *node;
    msurface_t *surf;
    ...
    for (;;) {
        if (node->visframe != r_visframecount) goto NodeUp;

        if (node->clipflags) {
            /* Clip the node against the frustum */
            for (i = 0; i < 4; i++) {
                if (!(node->clipflags & (1 << i))) continue;
                ...
                if (side == PSIDE_FRONT) node->clipflags &= ~(1 << i);
            }
        }

        if (node->contents < 0) goto NodeUp;
        ...
        for (i = 0; i < node->numsurfaces; i++, surf++) {
            ...
            for (j = 0; j < 4; j++) {
                if (!(node->clipflags & (1 << j))) continue;
                ...
                if (side == PSIDE_FRONT) surf->clipflags &= ~(1 << j);
            }
            if (j < 4) continue;
            ...
            if (surf->flags & SURF_PLANEBACK) {
                if (dist > -BACKFACE_EPSILON) surf->clipflags = BMODEL_FULLY_CLIPPED;
            } else {
                if (dist < BACKFACE_EPSILON) surf->clipflags = BMODEL_FULLY_CLIPPED;
            }
        }
        ...
        if (node->children[0]->contents == CONTENTS_SOLID) goto DownRight;
    ...
    DownRight:
        ...
        if (node->children[1]->contents == CONTENTS_SOLID) goto NodeUp;
    ...
    NodeUp:
        ...
        while (node->parent && node == node->parent->children[1]) node = node->parent;
...
=============
*/
static void R_CullSubmodelSurfaces(const brushmodel_t *submodel, const vec3_t vieworg,
                                   int clipflags) {
    int i, j, side;
    msurface_t *surf;
    mplane_t *plane;
    ...
    for (i = 0; i < submodel->nummodelsurfaces; i++, surf++) {
        ...
        for (j = 0; j < 4; j++) {
            if (!(surf->clipflags & (1 << j))) continue;
            plane = &view_clipplanes[j].plane;
            side = BoxOnPlaneSide(surf->mins, surf->maxs, plane);
            if (side == PSIDE_BACK) {
                surf->clipflags = BMODEL_FULLY_CLIPPED;
...
static void R_DrawEntitiesOnList(void) {
    ...
    for (i = 0; i < cl_numvisedicts; i++) {
        e = &cl_visedicts[i];
#ifdef NQ_HACK
        if (e == &cl_entities[cl.viewentity]) continue;  // don't draw the player
#endif
        switch (e->model->type) {
            case mod_sprite:
...
static void R_DrawViewModel(void) {
    ...
    if (cl.stats[STAT_ITEMS] & IT_INVISIBILITY) return;
    ...
    if (j < 24) j = 24;  // always give some light on gun
...
=============
*/
static int R_ModelCheckBBox(const entity_t *e, model_t *model, const vec3_t mins,
                            const vec3_t maxs) {
    ...
    if (e->angles[0] || e->angles[1] || e->angles[2]) {
        for (i = 0; i < 4; i++) {
            ...
            if (dist <= -model->radius) return BMODEL_FULLY_CLIPPED;

            if (dist <= model->radius) clipflags |= (1 << i);
        }
    } else {
        for (i = 0; i < 4; i++) {
            side = BoxOnPlaneSide(mins, maxs, &view_clipplanes[i].plane);
            if (side == PSIDE_BACK) return BMODEL_FULLY_CLIPPED;
            if (side == PSIDE_BOTH) clipflags |= (1 << i);
        }
    }
...
static void R_DrawBEntitiesOnList(void) {
    ...
    for (i = 0; i < cl_numvisedicts; i++) {
        ...
        if (entity->model->type != mod_brush) continue;
        ...
        if (clipflags == BMODEL_FULLY_CLIPPED) continue;
        ...
        if (brushmodel->firstmodelsurface != 0) {
            for (j = 0; j < MAX_DLIGHTS; j++) {
                if ((cl_dlights[j].die < cl.time) || (!cl_dlights[j].radius)) continue;
...
================
*/
__attribute__((noinline)) static void R_EdgeDrawing(void) {
...
static void R_RenderView_(void) {
    ...
    if (r_timegraph.value || r_speeds.value || r_dspeeds.value) r_time1 = Sys_DoubleTime();
    ...
    if (!r_worldentity.model || !cl.worldmodel) Sys_Error("%s: NULL worldmodel", __func__);
...
#ifdef QW_HACK
    if (r_netgraph.value) R_NetGraph();

    if (r_zgraph.value) R_ZGraph();
#endif
...
void R_RenderView(void) {
    ...
    if ((intptr_t)(&dummy) & 3) Sys_Error("Stack is missaligned");

    if ((intptr_t)(&r_warpbuffer) & 3) Sys_Error("Globals are missaligned");
