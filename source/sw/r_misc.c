...
void R_LineGraph(int x, int y, int h) {
    ...
    for (i = 0; i < h; i++, dest -= vid.rowbytes * 2) dest[0] = color;
}
...
void R_PrintTimes(void) {
    ...
    Con_Printf("%5.1f ms %3i/%3i/%3i poly %3i surf\n", ms, c_faceclip, r_polycount,
               r_drawnpolycount, c_surf);
    c_surf = 0;
}
...
void R_PrintDSpeeds(void) {
    ...
    Con_Printf("%3i %4.1fp %3iw %4.1fb %3is %4.1fe %4.1fv\n", (int)ms, dp_time, (int)rw_time,
               db_time, (int)se_time, de_time, dv_time);
}
...
=============
*/
void R_PrintAliasStats(void) { Con_Printf("%3i polygon model drawn\n", r_amodels_drawn); }
...
// don't allow cheats in multiplayer
    ...
    if (r_numedges.value) {
        ...
        if (edgecount > r_maxedgesseen) r_maxedgesseen = edgecount;
