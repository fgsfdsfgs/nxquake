...
static qboolean R_EmitCachedEdge(void) {
    ...
    if ((byte *)edge_p - (byte *)r_edges <= r_pedge->cachededgeoffset) return false;
    edge = (edge_t *)((byte *)r_edges + r_pedge->cachededgeoffset);
    if (edge->owner != r_pedge) return false;
...
void R_RenderFace(const entity_t *e, msurface_t *fa, int clipflags) {
    ...
    for (i = 0; i < fa->numedges; i++) {
        ...
        if (lindex > 0) {
            ...
            R_ClipEdge(&brushmodel->vertexes[r_pedge->v[0]], &brushmodel->vertexes[r_pedge->v[1]],
                       pclip);
            ...
            cacheoffset = (byte *)edge_p - (byte *)r_edges;
            r_leftclipped = r_rightclipped = false;
            R_ClipEdge(&brushmodel->vertexes[r_pedge->v[1]], &brushmodel->vertexes[r_pedge->v[0]],
                       pclip);
            r_pedge->cachededgeoffset = cacheoffset;
    ...
    surface_p->d_ziorigin =
        p_normal[2] * distinv - xcenter * surface_p->d_zistepu - ycenter * surface_p->d_zistepv;
