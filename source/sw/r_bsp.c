...
static void R_RecursiveClipBPoly(const entity_t *e, bedge_t *pedges, mnode_t *pnode,
    ...
    for (; pedges; pedges = pnextedge) {
        ...
        if (side != lastside) {
            ...
            ptvert->position[0] =
                plastvert->position[0] + frac * (pvert->position[0] - plastvert->position[0]);
            ptvert->position[1] =
                plastvert->position[1] + frac * (pvert->position[1] - plastvert->position[1]);
            ptvert->position[2] =
                plastvert->position[2] + frac * (pvert->position[2] - plastvert->position[2]);
...
void R_DrawSolidClippedSubmodelPolygons(const entity_t *entity) {
    ...
    for (i = 0; i < numsurfaces; i++, surf++) {
        if (surf->clipflags == BMODEL_FULLY_CLIPPED) continue;
