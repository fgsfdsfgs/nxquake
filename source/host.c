...
void Host_EndGame(const char *message, ...) {
    ...
    if (cls.state == ca_dedicated) Sys_Error("%s: %s", __func__, string);  // dedicated servers exit
...
void Host_Error(const char *error, ...) {
    ...
    if (inerror) Sys_Error("%s: recursively entered", __func__);
    inerror = true;
...
void Host_InitLocal(void) {
    ...
    if (COM_CheckParm("-developer")) Cvar_SetValue("developer", 1);
...
void Host_ShutdownServer(qboolean crash) {
    ...
    do {
        ...
        for (i = 0; i < svs.maxclients; i++, client++) {
            if (!client->active || !client->message.cursize) continue;
            if (NET_CanSendMessage(client->netconnection)) {
                NET_SendMessage(client->netconnection, &client->message);
    ...
    if (count) Con_Printf("%s: NET_SendToAll failed for %u clients\n", __func__, count);

    client = svs.clients;
    for (i = 0; i < svs.maxclients; i++, client++)
        if (client->active) SV_DropClient(client, crash);
