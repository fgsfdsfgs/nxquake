...
int CL_PlayerEntity(const entity_t *e) {
    ...
    if (offset >= 1 && offset <= cl.maxclients) return offset;

    /* ...but if not, try to find a match */
    for (i = 1; i <= cl.maxclients; i++) {
        /* Compare just the top of the struct, up to the lerp info */
        if (!memcmp(e, &cl_entities[i], offsetof(entity_t, previouspose))) return i;
    }
...
void CL_ClearState(void) {
    ...
    for (i = 0; i < MAX_EFRAGS - 1; i++) cl.free_efrags[i].entnext = &cl.free_efrags[i + 1];
...
void CL_PrintEntities_f(void) {
    ...
    for (i = 0, ent = cl_entities; i < cl.num_entities; i++, ent++) {
        ...
        Con_Printf("%s:%2i  (%5.1f,%5.1f,%5.1f) [%5.1f %5.1f %5.1f]\n", ent->model->name,
                   ent->frame, ent->origin[0], ent->origin[1], ent->origin[2], ent->angles[0],
...
void CL_Name_f(void) {
    ...
    if (!strcmp(cl_name.string, new_name)) return;
    ...
    for (i = 1, ent = cl_entities + 1; i < cl.num_entities; i++, ent++) {
        ...
        if (ent->forcelink) {
            ...
            for (j = 0; j < 3; j++) {
                ...
                if (delta[j] > 100 || delta[j] < -100) f = 1; /* assume a teleport and don't lerp */
            }
