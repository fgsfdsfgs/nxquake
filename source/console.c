...
int Con_GetWidth(void) { return con_linewidth; }
...
================
*/
void Con_Clear_f(void) { memset(con_main.text, ' ', CON_TEXTSIZE); }
        ...
        for (i = 0; i < numlines; i++) {
            for (j = 0; j < numchars; j++) {
                c->text[(con_totallines - 1 - i) * con_linewidth + j] =
                    tbuf[((c->current - i + oldtotallines) % oldtotallines) * oldwidth + j];
            }
        }
        Con_ClearNotify();
    }
...
================
*/
void Con_CheckResize(void) { Con_Resize(&con_main); }
...
void Con_Print(const char *txt) {
    ...
    if (txt[0] == 1 || txt[0] == 2) {
        ...
        txt++;
#ifdef NQ_HACK
        if (txt[0] == 1) S_LocalSound("misc/talk.wav");  // play talk wav
#endif
    } else
        mask = 0;
    ...
    while ((c = *txt)) {
        ...
        // word wrap
        if (l != con_linewidth && (con->x + l > con_linewidth)) con->x = 0;
        ...
        if (!con->x) {
            ...
            if (con->current >= 0) con_times[con->current % NUM_CON_TIMES] = realtime;
        }
        ...
        switch (c) {
            ...
            default:  // display character and advance
                ...
                con->x++;
                if (con->x >= con_linewidth) con->x = 0;
                break;
        }
    }
}
...
void Con_Printf(const char *fmt, ...) {
    ...
    if (debuglog) Sys_DebugLog(va("%s/qconsole.log", com_gamedir), "%s", msg);
...
void Con_DrawInput(void) {
    ...
    for (i = key_linepos + 1; i < con_linewidth; i++) text[i] = ' ';
    ...
    // draw it
    y = con_vislines - 22;
    for (i = 0; i < con_linewidth; i++) Draw_Character((i + 1) << 3, y, text[i]);
...
void Con_DrawNotify(void) {
    ...
    for (i = con->current - NUM_CON_TIMES + 1; i <= con->current; i++) {
        ...
        for (x = 0; x < con_linewidth; x++) Draw_Character((x + 1) << 3, v, text[x]);
...
static void Con_DrawDLBar(void) {
#ifdef QW_HACK
    char dlbar[256];
    const char *dlname;
    char *buf;
...
void Con_DrawConsole(int lines) {
    ...
    if (con->display != con->current) {
        ...
        for (x = 0; x < con_linewidth; x += 4) Draw_Character((x + 1) << 3, y, '^');
        y -= 8;
        rows--;
    }
    ...
    for (i = 0; i < rows; i++, y -= 8, row--) {
        if (row < 0) break;
        if (con->current - row >= con_totallines) break;  // past scrollback wrap point

        text = con->text + (row % con_totallines) * con_linewidth;
        for (x = 0; x < con_linewidth; x++) Draw_Character((x + 1) << 3, y, text[x]);
    }
...
void Con_NotifyBox(char *text) {
    ...
    Con_Printf(
        "\35\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36"
...
static void Con_ShowTree_Populate(struct rb_node *n) {
    if (n) {
