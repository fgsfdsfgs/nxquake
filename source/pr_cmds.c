...
static void PF_makevectors(void) {
    AngleVectors(G_VECTOR(OFS_PARM0), pr_global_struct->v_forward, pr_global_struct->v_right,
...
#ifdef NQ_HACK
static void SetMinMaxSize(edict_t *entity, const vec_t *min, const vec_t *max, qboolean rotate) {
    vec3_t rmin, rmax;
    vec3_t bounds[2];
    vec_t xvec[2], yvec[2];
    vec_t angle;
...
static void PF_setmodel(void) {
    ...
    for (i = 0, check = sv.model_precache; *check; i++, check++)
        if (!strcmp(*check, modelname)) break;
...
*/
    ...
    static void PF_WriteSoundNum_Static(sizebuf_t * sb, int c) {
        switch (sv.protocol) {
...
=================
*/
    __attribute__((noreturn)) static void PF_break(void) {
...
// cycle to the next one
        ...
        for (;; entnum++) {
#ifdef NQ_HACK
            if (entnum == svs.maxclients + 1) entnum = 1;
#endif
#ifdef QW_HACK
            if (entnum == MAX_CLIENTS + 1) entnum = 1;
#endif

            ent = EDICT_NUM(entnum);

            if (entnum == check) break;  // didn't find anything else
...
=================
*/
    static void PF_localcmd(void) { Cbuf_AddText("%s", G_STRING(OFS_PARM0)); }
...
*/
    ...
    static void PF_Find(void) {
        ...
        if (!s) PR_RunError("%s: bad search string", __func__);
    ...
    static void PR_CheckEmptyString(const char *s) {
        if (s[0] <= ' ') PR_RunError("%s: Bad string", __func__);
    }
...
*/
            static void PF_droptofloor(void) {
                ...
                ground = SV_TraceMoveEntity(entity, entity->v.origin, end, MOVE_NORMAL, &trace);
...
*/
            ...
            static void PF_aim(void) {
                ...
                if (target && target->v.takedamage == DAMAGE_AIM &&
                    (!teamplay.value || ent->v.team <= 0 || ent->v.team != target->v.team)) {
                    VectorCopy(pr_global_struct->v_forward, G_VECTOR(OFS_RETURN));
                    return;
                }
...
#define MSG_MULTICAST 4 /* for multicast()              */
            ...
            static client_t *Write_GetClient(void) {
                ...
                ent = PROG_TO_EDICT(pr_global_struct->msg_entity);
                entnum = NUM_FOR_EDICT(ent);
                if (entnum < 1 || entnum > MAX_CLIENTS) PR_RunError("%s: not a client", __func__);
            ...
            static void PF_makestatic(void) {
                ...
                if (sv.protocol == PROTOCOL_VERSION_FITZ) {
                    ...
                    if ((int)ent->v.frame & 0xff00) bits |= B_FITZ_LARGEFRAME;
#if 0
...
*/
            ...
            static void PF_Fixme(void) { PR_RunError("unimplemented bulitin"); }
