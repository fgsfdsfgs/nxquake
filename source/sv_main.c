...
#define PROT(v, n, d) \
    { .version = v, .name = n, .description = d }
    ...
    if (Cmd_Argc() == 1) {
        ...
        if (!strcasecmp(Cmd_Argv(1), "list")) {
            Con_Printf(
                ...
                "-------  ----  -----------\n");
            for (i = 0; i < ARRAY_SIZE(sv_protocols); i++) {
                Con_Printf("%7d  %-4s  %s\n", sv_protocols[i].version, sv_protocols[i].name,
                           sv_protocols[i].description);
            }
        } else {
            int v = Q_atoi(Cmd_Argv(1));
            for (i = 0; i < ARRAY_SIZE(sv_protocols); i++) {
                if (sv_protocols[i].version == v) break;
                if (!strcasecmp(sv_protocols[i].name, Cmd_Argv(1))) break;
            }
            ...
            if (sv_protocol != sv_protocols[i].version) {
                ...
                if (sv.active)
                    Con_Printf(
                        "change will not take effect until the next "
                        "level load.\n");
            }
        }
    } else {
    ...
    if (root) {
        *root = STREE_ROOT;
        ...
        for (i = 0; i < ARRAY_SIZE(sv_protocols); i++) {
            ...
            if (arg_len && !strncmp(digits, arg, arg_len)) STree_InsertAlloc(root, digits, true);
        }
    }
    return root;
}
...
void SV_StartParticle(vec3_t org, vec3_t dir, int color, int count) {
     ...
     */
    if (sv.datagram.cursize > MAX_DATAGRAM - 12) return;
...
void SV_StartSound(edict_t *entity, int channel, const char *sample, int volume,
    ...
    if (volume < 0 || volume > 255) Sys_Error("%s: volume = %i", __func__, volume);

    if (attenuation < 0 || attenuation > 4)
        Sys_Error("%s: attenuation = %f", __func__, attenuation);

    if (channel < 0 || channel > 7) Sys_Error("%s: channel = %i", __func__, channel);

    /*
     * Drop silently if there is no room
     * FIXME - does not take into account MTU...
     */
    if (sv.datagram.cursize > MAX_DATAGRAM - 14) return;
...
void SV_SendServerinfo(client_t *client) {
    ...
    MSG_WriteStringf(&client->message, "%c\nVERSION TyrQuake-%s SERVER (%i CRC)", 2,
    ...
    for (s = sv.model_precache + 1; *s; s++) MSG_WriteString(&client->message, *s);
    MSG_WriteByte(&client->message, 0);

    for (s = sv.sound_precache + 1; *s; s++) MSG_WriteString(&client->message, *s);
    MSG_WriteByte(&client->message, 0);
...
void SV_ConnectClient(int clientnum) {
    ...
    if (sv.loadgame) memcpy(spawn_parms, client->spawn_parms, sizeof(spawn_parms));
...
void SV_CheckForNewClients(void) {
    ...
    while (1) {
        ...
        if (i == svs.maxclients) Sys_Error("%s: no free clients", __func__);
    ...
    for (e = 1; e < sv.num_edicts; e++, ent = NEXT_EDICT(ent)) {
        ...
        if (ent != clent) {
            ...
            for (i = 0; i < ent->num_leafs; i++)
                if (Mod_TestLeafBit(pvs, ent->leafnums[i])) break;
        ...
        if (ent->baseline.skinnum != ent->v.skin) bits |= U_SKIN;
        ...
        if (sv.protocol == PROTOCOL_VERSION_FITZ) {
            if ((bits & U_FRAME) && ((int)ent->v.frame & 0xff00)) bits |= U_FITZ_FRAME2;
            if ((bits & U_MODEL) && ((int)ent->v.modelindex & 0xff00)) bits |= U_FITZ_MODEL2;
            /* FIXME - Add the U_LERPFINISH bit */
            if (bits & 0x00ff0000) bits |= U_FITZ_EXTEND1;
            if (bits & 0xff000000) bits |= U_FITZ_EXTEND2;
        }
        ...
        if (bits & U_FITZ_EXTEND1) MSG_WriteByte(msg, bits >> 16);
        if (bits & U_FITZ_EXTEND2) MSG_WriteByte(msg, bits >> 24);
        ...
        if (bits & U_MODEL) SV_WriteModelIndex(msg, ent->v.modelindex, 0);
...
#endif
        if (bits & U_FITZ_FRAME2) MSG_WriteByte(msg, (int)ent->v.frame >> 8);
        if (bits & U_FITZ_MODEL2) MSG_WriteByte(msg, (int)ent->v.modelindex >> 8);
#if 0 /* FIXME */
...
==================
*/
void SV_WriteClientdataToMessage(edict_t *player, sizebuf_t *msg) {
    int bits;
    int i;
    edict_t *other;
    int items;
    ...
    if (player->v.fixangle) {
        ...
        for (i = 0; i < 3; i++) MSG_WriteAngle(msg, player->v.angles[i]);
        player->v.fixangle = 0;
    }

    bits = 0;

    if (player->v.view_ofs[2] != DEFAULT_VIEWHEIGHT) bits |= SU_VIEWHEIGHT;

    if (player->v.idealpitch) bits |= SU_IDEALPITCH;
    ...
    if ((int)player->v.flags & FL_ONGROUND) bits |= SU_ONGROUND;

    if (player->v.waterlevel >= 2) bits |= SU_INWATER;

    for (i = 0; i < 3; i++) {
        if (player->v.punchangle[i]) bits |= (SU_PUNCH1 << i);
        if (player->v.velocity[i]) bits |= (SU_VELOCITY1 << i);
    }

    if (player->v.weaponframe) bits |= SU_WEAPONFRAME;

    if (player->v.armorvalue) bits |= SU_ARMOR;

    // if (player->v.weapon)
    bits |= SU_WEAPON;

    if (sv.protocol == PROTOCOL_VERSION_FITZ) {
        if ((bits & SU_WEAPON) && (SV_ModelIndex(PR_GetString(player->v.weaponmodel)) & 0xff00))
            bits |= SU_FITZ_WEAPON2;
        if ((int)player->v.armorvalue & 0xff00) bits |= SU_FITZ_ARMOR2;
        if ((int)player->v.currentammo & 0xff00) bits |= SU_FITZ_AMMO2;
        if ((int)player->v.ammo_shells & 0xff00) bits |= SU_FITZ_SHELLS2;
        if ((int)player->v.ammo_nails & 0xff00) bits |= SU_FITZ_NAILS2;
        if ((int)player->v.ammo_rockets & 0xff00) bits |= SU_FITZ_ROCKETS2;
        if ((int)player->v.ammo_cells & 0xff00) bits |= SU_FITZ_CELLS2;
        if ((bits & SU_WEAPONFRAME) && ((int)player->v.weaponframe & 0xff00))
...
#endif
        if (bits & 0x00ff0000) bits |= SU_FITZ_EXTEND1;
        if (bits & 0xff000000) bits |= SU_FITZ_EXTEND2;
    }
    ...
    if (bits & SU_FITZ_EXTEND1) MSG_WriteByte(msg, bits >> 16);
    if (bits & SU_FITZ_EXTEND2) MSG_WriteByte(msg, bits >> 24);

    if (bits & SU_VIEWHEIGHT) MSG_WriteChar(msg, player->v.view_ofs[2]);

    if (bits & SU_IDEALPITCH) MSG_WriteChar(msg, player->v.idealpitch);

    for (i = 0; i < 3; i++) {
        if (bits & (SU_PUNCH1 << i)) MSG_WriteChar(msg, player->v.punchangle[i]);
        if (bits & (SU_VELOCITY1 << i)) MSG_WriteChar(msg, player->v.velocity[i] / 16);
    }

    // [always sent]        if (bits & SU_ITEMS)
    MSG_WriteLong(msg, items);

    if (bits & SU_WEAPONFRAME) MSG_WriteByte(msg, player->v.weaponframe);
    if (bits & SU_ARMOR) MSG_WriteByte(msg, player->v.armorvalue);
    if (bits & SU_WEAPON)
    ...
    if (bits & SU_FITZ_ARMOR2) MSG_WriteByte(msg, (int)player->v.armorvalue >> 8);
    if (bits & SU_FITZ_AMMO2) MSG_WriteByte(msg, (int)player->v.currentammo >> 8);
    if (bits & SU_FITZ_SHELLS2) MSG_WriteByte(msg, (int)player->v.ammo_shells >> 8);
    if (bits & SU_FITZ_NAILS2) MSG_WriteByte(msg, (int)player->v.ammo_nails >> 8);
    if (bits & SU_FITZ_ROCKETS2) MSG_WriteByte(msg, (int)player->v.ammo_rockets >> 8);
    if (bits & SU_FITZ_CELLS2) MSG_WriteByte(msg, (int)player->v.ammo_cells >> 8);
    if (bits & SU_FITZ_WEAPONFRAME2) MSG_WriteByte(msg, (int)player->v.weaponframe >> 8);
#if 0 /* FIXME - TODO */
...
void SV_UpdateToReliableMessages(void) {
    ...
    for (i = 0; i < svs.maxclients; i++, recipient++) {
        if (!recipient->active) continue;
        SZ_Write(&recipient->message, sv.reliable_datagram.data, sv.reliable_datagram.cursize);
    }
...
void SV_SendNop(client_t *client) {
    ...
    err = NET_SendUnreliableMessage(client->netconnection, &msg);
    /* if the message couldn't send, kick the client off */
    if (err == -1) SV_DropClient(client, true);
...
void SV_SendClientMessages(void) {
    ...
    for (i = 0; i < svs.maxclients; i++, client++) {
        ...
        if (client->spawned) {
            if (!SV_SendClientDatagram(client)) continue;
        } else {
            /*
            ...
            if (!client->sendsignon) {
                if (realtime - client->last_message > 5) SV_SendNop(client);
        ...
        if (!client->message.cursize && !client->dropasap) continue;
        if (!NET_CanSendMessage(client->netconnection)) continue;
...
void SV_CreateBaseline(void) {
    ...
    for (entnum = 0; entnum < sv.num_edicts; entnum++) {
        ...
        bits = 0;
        if (sv.protocol == PROTOCOL_VERSION_FITZ) {
            if (svent->baseline.modelindex & 0xff00) bits |= B_FITZ_LARGEMODEL;
            if (svent->baseline.frame & 0xff00) bits |= B_FITZ_LARGEFRAME;
#if 0 /* FIXME - TODO */
...
void SV_SendReconnect(void) {
    ...
    if (cls.state != ca_dedicated) Cmd_ExecuteString("reconnect\n", src_command);
}
...
void SV_SpawnServer(char *server) {
    ...
    client = svs.clients;
    for (i = 0; i < svs.maxclients; i++, client++)
        if (client->active) SV_SendServerinfo(client);
