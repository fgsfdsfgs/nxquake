...
mleaf_t *Mod_PointInLeaf(const brushmodel_t *model, const vec3_t point) {
    ...
    if (!model || !model->nodes) SV_Error("%s: bad model", __func__);
...
void Mod_AddLeafBits(leafbits_t *dst, const leafbits_t *src) {
    ...
    if (src->numleafs != dst->numleafs)
        SV_Error("%s: src->numleafs (%d) != dst->numleafs (%d)", __func__, src->numleafs,
                 dst->numleafs);

    srcblock = src->bits;
    dstblock = dst->bits;
    leafblocks = (src->numleafs + LEAFMASK) >> LEAFSHIFT;
    for (i = 0; i < leafblocks; i++) *dstblock++ |= *srcblock++;
}
...
static void Mod_InitPVSCache(int numleafs) {
    int i;
    int memsize;
    byte *leafmem;
...
static void Mod_DecompressVis(const byte *in, const brushmodel_t *model, leafbits_t *dest) {
    leafblock_t *out;
    int num_out;
    int shift;
    int count;
...
static void Mod_AddToFatPVS(const brushmodel_t *model, const vec3_t point, const mnode_t *node) {
...
static model_t *Mod_FindName(const char *name) {
    ...
    if (!name[0]) SV_Error("%s: NULL name", __func__);

    /* search the currently loaded models */
    brushmodel = loaded_models;
    while (brushmodel) {
        if (!strcmp(brushmodel->model.name, name)) break;
    ...
    if (!model) {
        ...
        while (model) {
            if (!strcmp(model->name, name)) break;
            model = model->next;
        }
    }
    if (!model) model = Mod_FindAliasName(name);
#endif
...
void *Mod_AllocName(int size, const char *modelname) {
...
static model_t *Mod_LoadModel(const char *name, qboolean crash) {
    ...
    if (!buf) {
        if (crash) SV_Error("%s: %s not found", __func__, name);
        return NULL;
    }
...
model_t *Mod_ForName(const char *name, qboolean crash) {
    ...
    if (model) {
        ...
        if (model->type != mod_alias) return model;
        if (Cache_Check(&model->cache)) return model;
...
=================
*/
static void Mod_LoadTextures(brushmodel_t *brushmodel, dheader_t *header) {
...
#ifndef SERVERONLY
        if (!strncmp(mt->name, "sky", 3)) R_InitSky(tx);
#ifdef GLQUAKE
        else {
    ...
    for (i = 0; i < lump->nummiptex; i++) {
        ...
        if (tx->anim_next) continue; /* Already sequenced */
...
#define ANIM_CYCLE 2
        ...
        for (j = 0; j < max; j++) {
            tx2 = anims[j];
            if (!tx2) SV_Error("Missing frame %i of %s", j, tx->name);
 ...
 */
static void *Mod_LoadBytes(brushmodel_t *brushmodel, dheader_t *header, int lumpnum) {
    const model_t *model = &brushmodel->model;
    const lump_t *headerlump = &header->lumps[lumpnum];
    const byte *in;
    byte *out;

    if (!headerlump->filelen) return NULL;
...
=================
*/
static void Mod_LoadLighting(brushmodel_t *brushmodel, dheader_t *header) {
...
=================
*/
static void Mod_LoadVisibility(brushmodel_t *brushmodel, dheader_t *header) {
...
=================
*/
static void Mod_LoadEntities(brushmodel_t *brushmodel, dheader_t *header) {
...
=================
*/
static void Mod_LoadVertexes(brushmodel_t *brushmodel, const dheader_t *header) {
    const model_t *model = &brushmodel->model;
...
=================
*/
static void Mod_LoadSubmodels(brushmodel_t *brushmodel, dheader_t *header) {
    const model_t *model = &brushmodel->model;
    ...
    for (i = 0; i < count; i++, in++, out++) {
        ...
        for (j = 0; j < MAX_MAP_HULLS; j++) out->headnode[j] = LittleLong(in->headnode[j]);
...
=================
*/
static void Mod_LoadEdges_BSP29(brushmodel_t *brushmodel, dheader_t *header) {
    const model_t *model = &brushmodel->model;
...
static void Mod_LoadEdges_BSP2(brushmodel_t *brushmodel, dheader_t *header) {
    const model_t *model = &brushmodel->model;
    const lump_t *headerlump = &header->lumps[LUMP_EDGES];
...
=================
*/
static void Mod_LoadTexinfo(brushmodel_t *brushmodel, dheader_t *header) {
    const model_t *model = &brushmodel->model;
    ...
    for (i = 0; i < count; i++, in++, out++) {
        ...
        if (!brushmodel->textures) {
            ...
            if (miptex >= brushmodel->numtextures) SV_Error("miptex >= brushmodel->numtextures");
...
================
*/
static void CalcSurfaceExtents(const brushmodel_t *brushmodel, msurface_t *surf) {
    ...
    for (i = 0; i < surf->numedges; i++) {
         ...
         */
        for (j = 0; j < 2; j++) {
            val = (long double)vertex->position[0] * texinfo->vecs[j][0] +
                  (long double)vertex->position[1] * texinfo->vecs[j][1] +
                  (long double)vertex->position[2] * texinfo->vecs[j][2] + texinfo->vecs[j][3];
...
static void CalcSurfaceBounds(const brushmodel_t *brushmodel, msurface_t *surf) {
    int i, j;
    const mvertex_t *vertex;
    ...
    for (i = 0; i < surf->numedges; i++) {
        ...
        for (j = 0; j < 3; j++) {
            if (surf->mins[j] > vertex->position[j]) surf->mins[j] = vertex->position[j];
            if (surf->maxs[j] < vertex->position[j]) surf->maxs[j] = vertex->position[j];
        }
    }
}

static void Mod_ProcessSurface(brushmodel_t *brushmodel, msurface_t *surf) {
...
=================
*/
static void Mod_LoadFaces_BSP29(brushmodel_t *brushmodel, dheader_t *header) {
    const model_t *model = &brushmodel->model;
    ...
    for (surfnum = 0; surfnum < count; surfnum++, in++, out++) {
        ...
        if (out->numedges <= 0)
            SV_Error("%s: bmodel %s has surface with no edges", __func__, model->name);
...
static void Mod_LoadFaces_BSP2(brushmodel_t *brushmodel, dheader_t *header) {
    const model_t *model = &brushmodel->model;
    const lump_t *headerlump = &header->lumps[LUMP_FACES];
...
=================
*/
static void Mod_LoadNodes_BSP29(brushmodel_t *brushmodel, dheader_t *header) {
    const model_t *model = &brushmodel->model;
...
static void Mod_LoadNodes_BSP2rmq(brushmodel_t *brushmodel, dheader_t *header) {
    const model_t *model = &brushmodel->model;
    const lump_t *headerlump = &header->lumps[LUMP_NODES];
...
static void Mod_LoadNodes_BSP2(brushmodel_t *brushmodel, dheader_t *header) {
    const model_t *model = &brushmodel->model;
    const lump_t *headerlump = &header->lumps[LUMP_NODES];
...
static void Mod_SetLeafFlags(const model_t *model, mleaf_t *leaf) {
#ifdef GLQUAKE
    int i;
...
#ifdef QW_HACK
    {
        ...
        snprintf(mapmodel, sizeof(mapmodel), "maps/%s.bsp", Info_ValueForKey(cl.serverinfo, "map"));
...
=================
*/
static void Mod_LoadLeafs_BSP29(brushmodel_t *brushmodel, dheader_t *header) {
    const model_t *model = &brushmodel->model;
    ...
    for (i = 0; i < count; i++, in++, out++) {
        ...
        out->firstmarksurface =
            brushmodel->marksurfaces + (uint16_t)LittleShort(in->firstmarksurface);
...
static void Mod_LoadLeafs_BSP2rmq(brushmodel_t *brushmodel, dheader_t *header) {
    const model_t *model = &brushmodel->model;
    const lump_t *headerlump = &header->lumps[LUMP_LEAFS];
    ...
    for (i = 0; i < count; i++, in++, out++) {
        ...
        out->contents = LittleLong(in->contents);
        out->firstmarksurface =
            brushmodel->marksurfaces + (uint32_t)LittleLong(in->firstmarksurface);
...
static void Mod_LoadLeafs_BSP2(brushmodel_t *brushmodel, dheader_t *header) {
    const model_t *model = &brushmodel->model;
    const lump_t *headerlump = &header->lumps[LUMP_LEAFS];
...
static void Mod_MakeClipHulls(brushmodel_t *brushmodel) {
    hull_t *hull;
...
=================
*/
static void Mod_LoadClipnodes_BSP29(brushmodel_t *brushmodel, dheader_t *header) {
    const model_t *model = &brushmodel->model;
    ...
    for (i = 0; i < count; i++, out++, in++) {
        ...
        for (j = 0; j < 2; j++) {
            ...
            if (out->children[j] > 0xfff0) out->children[j] -= 0x10000;
            if (out->children[j] >= count) SV_Error("%s: bad clipnode child number", __func__);
        }
    }
}

static void Mod_LoadClipnodes_BSP2(brushmodel_t *brushmodel, dheader_t *header) {
    const model_t *model = &brushmodel->model;
    const lump_t *headerlump = &header->lumps[LUMP_CLIPNODES];
...
=================
*/
static void Mod_MakeDrawHull(brushmodel_t *brushmodel) {
    const model_t *model = &brushmodel->model;
...
=================
*/
static void Mod_LoadMarksurfaces_BSP29(brushmodel_t *brushmodel, dheader_t *header) {
    const model_t *model = &brushmodel->model;
...
static void Mod_LoadMarksurfaces_BSP2(brushmodel_t *brushmodel, dheader_t *header) {
    const model_t *model = &brushmodel->model;
    const lump_t *headerlump = &header->lumps[LUMP_MARKSURFACES];
    const uint32_t *in;
    msurface_t **out;
...
=================
*/
static void Mod_LoadSurfedges(brushmodel_t *brushmodel, dheader_t *header) {
    const model_t *model = &brushmodel->model;
...
=================
*/
static void Mod_LoadPlanes(brushmodel_t *brushmodel, dheader_t *header) {
    const model_t *model = &brushmodel->model;
    ...
    for (i = 0; i < count; i++, in++, out++) {
        ...
        for (j = 0; j < 3; j++) {
            ...
            if (out->normal[j] < 0) signbits |= 1 << j;
        }
...
static float RadiusFromBounds(vec3_t mins, vec3_t maxs) {
    ...
    for (i = 0; i < 3; i++) corner[i] = qmax(fabs(mins[i]), fabs(maxs[i]));

    return Length(corner);
}

static void Mod_SetupSubmodels(brushmodel_t *world) {
...
static const char *BSPVersionString(int32_t version) {
...
static qboolean BSPVersionSupported(int32_t version) {
    switch (version) {
        case BSPVERSION:
        case BSP2VERSION:
        case BSP2RMQVERSION:
...
static void Mod_LoadBrushModel(brushmodel_t *brushmodel, void *buffer, size_t size) {
    ...
    if (!BSPVersionSupported(header->version))
        SV_Error("%s: %s has unsupported version %s", __func__, model->name,
    ...
    if (brushmodel->numleafs > pvscache_numleafs) {
        if (pvscache[0].leafbits)
            SV_Error("%s: %d allocated for visdata, but model %s has %d leafs", __func__,
                     pvscache_numleafs, model->name, brushmodel->numleafs);
...
void *Mod_Extradata(model_t *model) {
    ...
    Mod_LoadAliasModel(mod_loader, model, buffer);
    if (!model->cache.data) Sys_Error("%s: caching failed", __func__);
...
void Mod_Print(void) {
    ...
    for (model = Mod_AliasCache(); model; model = model->next) {
        Con_Printf("%*p : %s\n", (int)sizeof(void *) * 2 + 2, model->cache.data, model->name);
        alias++;
    }
...
static const boxhull_t boxhull_template = {
    .hull = {.clipnodes = box_clipnodes, .firstclipnode = 0, .lastclipnode = 5},
...
==================
*/
static qboolean Mod_TraceHull_r(const hull_t *hull, int nodenum, const float p1f, const float p2f,
    ...
    if (!Mod_TraceHull_r(hull, child, p1f, midf, p1, mid, trace)) return false;
    ...
    if (Mod_HullPointContents(hull, child, mid) != CONTENTS_SOLID) /* Go past the node */
