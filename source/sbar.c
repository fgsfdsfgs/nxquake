...
=============
*/
void Sbar_DrawPic(int x, int y, const qpic8_t *pic) {
...
=============
*/
void Sbar_DrawTransPic(int x, int y, const qpic8_t *pic) {
    if (cl.gametype == GAME_DEATHMATCH)
...
void Sbar_MiniDeathmatchOverlay(void) {
    ...
    if (vid.width < 512 || !sb_lines) return;
    ...
    for (line = 0; line < scoreboardlines; line++)
        if (fragsort[line] == cl.viewentity - 1) break;
    if (line == scoreboardlines) line = 0;
