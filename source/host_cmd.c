...
static void Host_Reconnect_f(void) {
    ...
    if (cls.state < ca_connected) Host_Error("Host_Reconnect_f: cls.state < ca_connected");
...
static void Host_Loadgame_f(void) {
    ...
    for (i = 0; i < NUM_SPAWN_PARMS; i++) svs.clients->spawn_parms[i] = spawn_parms[i];
...
static void Host_Startdemos_f(void) {
    ...
    for (i = 1; i < c + 1; i++) strncpy(cls.demos[i - 1], Cmd_Argv(i), sizeof(cls.demos[0]) - 1);
