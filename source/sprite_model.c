...
=================
*/
static const void *Mod_LoadSpriteFrame(const void *buffer, mspriteframe_t **ppframe,
...
=================
*/
static const void *Mod_LoadSpriteGroup(const void *buffer, mspritegroup_t **ppgroup,
                                       const char *loadname, int framenum) {
    ...
    *ppgroup = group;
    ...
    for (i = 0; i < numframes; i++, intervals++, dintervals++) {
        *intervals = LittleFloat(dintervals->interval);
        if (*intervals <= 0.0) Sys_Error("%s: interval <= 0", __func__);
    }
    buffer = dintervals;
...
void Mod_LoadSpriteModel(model_t *model, const void *buffer) {
    ...
    if (version != SPRITE_VERSION)
        Sys_Error("%s: %s has wrong version number (%i should be %i)", __func__, model->name,
...
==================
*/
const mspriteframe_t *Mod_GetSpriteFrame(const entity_t *entity, const msprite_t *sprite,
                                         float time) {
    ...
    if (framedesc->type == SPR_SINGLE) return framedesc->frame.frame;
    ...
    for (framenum = 0; framenum < numframes - 1; framenum++)
        if (intervals[framenum] > targettime) break;
