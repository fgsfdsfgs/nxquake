...
static struct stree_node *STree_AllocNode(void) {
...
static void *STree_AllocString(unsigned int length) {
    char *ret = NULL;
...
qboolean STree_Insert(struct stree_root *root, struct stree_node *node) {
    ...
    if (len > root->maxlen) root->maxlen = len;
    if (len < root->minlen) root->minlen = len;
 ...
 */
qboolean STree_InsertAlloc(struct stree_root *root, const char *s, qboolean alloc_str) {
    qboolean ret = false;
    struct stree_node *n;
    char *tmp;
...
static int ST_node_match(struct rb_node *n, const char *str, int min_match, int max_match) {
    if (n) {
        max_match = ST_node_match(n->rb_left, str, min_match, max_match);

        /* How much does this node match */
        while (max_match > min_match) {
            if (!strncasecmp(str, stree_entry(n)->string, max_match)) break;
            max_match--;
        }
...
static int STree_MaxDepth(struct stree_root *root) { return 2 * Q_log2(root->entries + 1); }

static void STree_StackInit(struct stree_root *root) {
    ...
    if (!root->stack)
        Sys_Error("%s: not enough mem for stack! (%i)", __func__, STree_MaxDepth(root));
}
...
static void STree_StackPush(struct stree_root *root, struct rb_node *n) {
    struct stree_stack *s = root->stack;
    assert(s->depth < s->max_depth);
    s->stack[s->depth++] = n;
}

static struct rb_node *STree_StackPop(struct stree_root *root) {
    struct stree_stack *s = root->stack;
    assert(s->depth > 0);
 ...
 * next node.
 */
void STree_ForEach_After__(struct stree_root *root, struct stree_node **n, const char *s) {
    struct rb_node *p;
    struct stree_node *node;
    int cmp;
...
void STree_Completions(struct stree_root *out, struct stree_root *in, const char *s) {
    ...
    while (rb) {
        ...
        if (!cmp) {
            ...
            if (rb->rb_left) {
                if (rb->rb_right) STree_StackPush(in, rb->rb_right);
                rb = rb->rb_left;
            } else {
                rb = rb->rb_right;
            }
        ...
        if (!rb && in->stack->depth > 0) rb = STree_StackPop(in);
    }
