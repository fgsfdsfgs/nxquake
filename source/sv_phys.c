...
==================
*/
static void SV_Impact(const edict_t *e1, const edict_t *e2) {
...
static int SV_FlyMove(edict_t *ent, float time, trace_t *steptrace) {
    ...
    for (bumpcount = 0; bumpcount < numbumps; bumpcount++) {
        ...
        ground = SV_TraceMoveEntity(ent, ent->v.origin, end, MOVE_NORMAL, &trace);
        ...
        if (!ground) SV_Error("%s: !ground", __func__);
        ...
        if (!trace.plane.normal[2]) {
            ...
            if (steptrace) /* save for player extrafriction */
                *steptrace = trace;
        }
...
============
*/
static const edict_t *SV_PushEntity(edict_t *ent, const vec3_t push, trace_t *trace) {
    vec3_t end;
    movetype_t movetype;
    ...
    if (blocker) SV_Impact(ent, blocker);
...
============
*/
static qboolean SV_Push(edict_t *pusher, const vec3_t move) {
    int i;
    edict_t *check, *block;
    vec3_t mins, maxs;
    vec3_t pushorig;
    ...
    for (i = 1; i < sv.num_edicts; i++, check = NEXT_EDICT(check)) {
        ...
        if (check->v.movetype != MOVETYPE_WALK) check->v.flags = (int)check->v.flags & ~FL_ONGROUND;
#endif
...
static void SV_PushMove(edict_t *pusher, float movetime) {
    ...
    for (i = 0; i < 3; i++) move[i] = pusher->v.velocity[i] * movetime;
...
=============
*/
static void SV_CheckStuck(edict_t *player) {
    int i, j;
    int z;
    vec3_t org;
...
=============
*/
static qboolean SV_CheckWater(edict_t *player) {
    vec3_t point;
    int contents;
    ...
    if (contents <= CONTENTS_WATER) {
        ...
        if (contents <= CONTENTS_WATER) {
            ...
            contents = SV_PointContents(point);
            if (contents <= CONTENTS_WATER) player->v.waterlevel = 3;
        }
    }
...
============
*/
static void SV_WallFriction(edict_t *player, trace_t *trace) {
...
======================
*/
static int SV_TryUnstick(edict_t *player, const vec3_t oldvel) {
    int i;
    vec3_t oldorg;
    vec3_t dir;
    int clip;
...
*/
#define STEPSIZE 18
static void SV_WalkMove(edict_t *player) {
    ...
    if (!(clip & MOVE_CLIP_WALL)) return;

    /* don't stair up while jumping */
    if (!oldonground && player->v.waterlevel == 0) return;

    /* gibbed by a trigger */
    if (player->v.movetype != MOVETYPE_WALK) return;

    if (sv_nostep.value) return;

    if ((int)player->v.flags & FL_WATERJUMP) return;
    ...
    if (clip & MOVE_CLIP_WALL) SV_WallFriction(player, &trace);
...
================
*/
static void SV_Physics_Client(edict_t *player, int playernum) {
    if (!svs.clients[playernum - 1].active) return;
    ...
    switch ((int)player->v.movetype) {
        ...
        case MOVETYPE_NOCLIP:
            if (!SV_RunThink(player)) return;
            VectorMA(player->v.origin, host_frametime, player->v.velocity, player->v.origin);
            break;
...
#ifdef QW_HACK
    if (ent->v.velocity[2] > 0) ent->v.flags = (int)ent->v.flags & ~FL_ONGROUND;
#endif
...
static void SV_RunEntity(edict_t *ent) {
    if (ent->v.lastruntime == (float)realtime) return;
...
void SV_RunNewmis(void) {
    ...
    if (!pr_global_struct->newmis) return;
...
void SV_Physics(void) {
    ...
    if (host_frametime < sv_mintic.value) return;
    if (host_frametime > sv_maxtic.value) host_frametime = sv_maxtic.value;
    old_time = realtime;
    ...
    for (i = 0; i < sv.num_edicts; i++, ent = NEXT_EDICT(ent)) {
        ...
        if (pr_global_struct->force_retouch) /* force retouch even for stationary */
            SV_LinkEdict(ent, true);
