...
restart:
    ...
    if (lightlevel >= 0) return lightlevel; /* hit something */

    if ((back < 0) == side) return -1; /* didn't hit anything */
...
int R_LightPoint(const vec3_t point) {
    ...
    if (!cl.worldmodel->lightdata) return 255;
    ...
    if (lightlevel == -1) lightlevel = 0;

#ifndef GLQUAKE
    if (lightlevel < r_refdef.ambientlight) lightlevel = r_refdef.ambientlight;
#endif
