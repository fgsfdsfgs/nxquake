...
================
*/
static const hull_t *SV_HullForEntity(const edict_t *ent, const vec3_t mins, const vec3_t maxs,
...
====================
*/
static void SV_AddLinksToPhysents_r(const areanode_t *node, const edict_t *player,
    ...
    for (link = solids->next; link != solids; link = next) {
        ...
        if (check->v.owner == playernum) continue;
        if (check->v.solid == SOLID_BSP || check->v.solid == SOLID_BBOX ||
            check->v.solid == SOLID_SLIDEBOX) {
            if (check == player) continue;
            for (i = 0; i < 3; i++)
                if (check->v.absmin[i] > maxs[i] || check->v.absmax[i] < mins[i]) break;
            if (i != 3) continue;
            if (pestack->numphysent == MAX_PHYSENTS) return;
...
void SV_AddLinksToPhysents(const edict_t *player, const vec3_t mins, const vec3_t maxs,
                           physent_stack_t *pestack) {
...
static areanode_t *SV_CreateAreaNode(int depth, const vec3_t mins, const vec3_t maxs) {
    ...
    if (sv_numareanodes == AREA_NODES) SV_Error("%s: sv_numareanodes == AREA_NODES", __func__);
...
void SV_UnlinkEdict(edict_t *ent) {
    ...
    RemoveLink(&ent->area);
    if (sv_link_next && *sv_link_next == &ent->area) *sv_link_next = ent->area.next;
    if (sv_link_prev && *sv_link_prev == &ent->area) *sv_link_prev = ent->area.prev;
...
====================
*/
static void SV_TouchLinks(edict_t *ent, const areanode_t *node) {
    link_t *link, *next;
...
static void SV_FindTouchedLeafs(edict_t *ent, const mnode_t *node) {
    ...
    if (sides & PSIDE_FRONT) SV_FindTouchedLeafs(ent, node->children[0]);
    if (sides & PSIDE_BACK) SV_FindTouchedLeafs(ent, node->children[1]);
}
...
edict_t *SV_TestEntityPosition(const edict_t *ent) {
    ...
    if (trace.startsolid) ret = sv.edicts;
...
==================
*/
static void SV_ClipToEntity(const edict_t *ent, const vec3_t start, const vec3_t mins,
...
====================
*/
static const edict_t *SV_ClipToLinks_r(const edict_t *clipent, const areanode_t *node,
    ...
    for (link = solids->next; link != solids; link = next) {
        ...
        if (clip->move.mins[0] > touch->v.absmax[0] || clip->move.mins[1] > touch->v.absmax[1] ||
            clip->move.mins[2] > touch->v.absmax[2] || clip->move.maxs[0] < touch->v.absmin[0] ||
            clip->move.maxs[1] < touch->v.absmin[1] || clip->move.maxs[2] < touch->v.absmin[2])
            continue;
        ...
        if (trace->allsolid) return clipent;
        if (clip->passedict) {
        ...
        SV_ClipToEntity(touch, clip->start, clipbounds->mins, clipbounds->maxs, clip->end,
                        &stacktrace);
...
static const edict_t *SV_ClipToLinks(const areanode_t *node, const moveclip_t *clip,
                                     trace_t *trace) {
...
==================
*/
static void SV_MoveBounds(const bounds_t *object, const vec3_t start, const vec3_t end,
                          bounds_t *move) {
    int i;
...
==================
*/
const edict_t *SV_TraceMove(const vec3_t start, const vec3_t mins, const vec3_t maxs,
    ...
    if (!clipent && clipworld) clipent = sv.edicts;
