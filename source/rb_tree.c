...
static void __rb_erase_color(struct rb_node *node, struct rb_node *parent, struct rb_root *root) {
    ...
    while ((!node || node->rb_color == QRB_BLACK) && node != root->rb_node) {
        if (parent->rb_left == node) {
            ...
            if ((!other->rb_left || other->rb_left->rb_color == QRB_BLACK) &&
                (!other->rb_right || other->rb_right->rb_color == QRB_BLACK)) {
                other->rb_color = QRB_RED;
                node = parent;
                parent = node->rb_parent;
            } else {
                if (!other->rb_right || other->rb_right->rb_color == QRB_BLACK) {
                    register struct rb_node *o_left;
                    if ((o_left = other->rb_left)) o_left->rb_color = QRB_BLACK;
                    other->rb_color = QRB_RED;
            ...
            if ((!other->rb_left || other->rb_left->rb_color == QRB_BLACK) &&
                ...
                if (!other->rb_left || other->rb_left->rb_color == QRB_BLACK) {
                    ...
                    if ((o_right = other->rb_right)) o_right->rb_color = QRB_BLACK;
                    other->rb_color = QRB_RED;
    ...
    if (node) node->rb_color = QRB_BLACK;
}
...
color:
    if (color == QRB_BLACK) __rb_erase_color(child, parent, root);
}
