...
static const char *svc_strings[] = {
    ...
    "",  // 38
    "",  // 39
    "svc_fitz_bf", "svc_fitz_fog", "svc_fitz_spawnbaseline2", "svc_fitz_spawnstatic2",
...
void CL_ParseStartSoundPacket(void) {
    ...
    for (i = 0; i < 3; i++) pos[i] = MSG_ReadCoord();
...
void CL_KeepaliveMessage(void) {
    ...
    do {
        ...
        switch (ret) {
            ...
            case 2:
                if (MSG_ReadByte() != svc_nop) Host_Error("%s: datagram wasn't a nop", __func__);
                break;
            default:
...
void CL_ParseServerInfo(void) {
    ...
    for (nummodels = 1;; nummodels++) {
        ...
        if (nummodels == max_models(cl.protocol)) {
            Host_Error("Server sent too many model precaches (max = %d)", max_models(cl.protocol));
            return;
        }
    ...
    for (numsounds = 1;; numsounds++) {
        ...
        if (numsounds == max_sounds(cl.protocol)) {
            Host_Error("Server sent too many sound precaches (max = %d)", max_sounds(cl.protocol));
            return;
        }
...
static int CL_ReadModelIndex(unsigned int bits) {
    switch (cl.protocol) {
        ...
        case PROTOCOL_VERSION_FITZ:
            if (bits & B_FITZ_LARGEMODEL) return MSG_ReadShort();
            return MSG_ReadByte();
        default:
            Host_Error("%s: Unknown protocol version (%d)\n", __func__, cl.protocol);
    }
...
static int CL_ReadModelFrame(unsigned int bits) {
    switch (cl.protocol) {
        ...
        case PROTOCOL_VERSION_BJP3:
            return MSG_ReadByte();
        case PROTOCOL_VERSION_FITZ:
            if (bits & B_FITZ_LARGEFRAME) return MSG_ReadShort();
            return MSG_ReadByte();
        default:
            Host_Error("%s: Unknown protocol version (%d)\n", __func__, cl.protocol);
    }
...
void CL_ParseUpdate(unsigned int bits) {
    ...
    if (cl.protocol == PROTOCOL_VERSION_FITZ) {
        if (bits & U_FITZ_EXTEND1) bits |= MSG_ReadByte() << 16;
        if (bits & U_FITZ_EXTEND2) bits |= MSG_ReadByte() << 24;
    }
    ...
    if (bits & U_MODEL) {
        ...
        if (modnum >= max_models(cl.protocol)) Host_Error("CL_ParseModel: bad modnum");
    } else
   ...
   currententities list or haven't been updated for a while */
    ...
    if (cl.protocol == PROTOCOL_VERSION_FITZ) {
        ...
        if (bits & U_FITZ_FRAME2) ent->frame = (ent->frame & 0xFF) | (MSG_ReadByte() << 8);
        if (bits & U_FITZ_MODEL2) modnum = (modnum & 0xFF) | (MSG_ReadByte() << 8);
...
==================
*/
static void CL_ParseBaseline(entity_t *ent, unsigned int bits) {
    int i;
...
void CL_ParseClientdata(void) {
    ...
    if (bits & SU_FITZ_EXTEND1) bits |= MSG_ReadByte() << 16;
    if (bits & SU_FITZ_EXTEND2) bits |= MSG_ReadByte() << 24;
    ...
    if (cl.stats[STAT_ITEMS] != i) {  // set flash times
        Sbar_Changed();
        for (j = 0; j < 32; j++)
            if ((i & (1 << j)) && !(cl.stats[STAT_ITEMS] & (1 << j))) cl.item_gettime[j] = cl.time;
    ...
    /* FITZ Protocol */
    if (bits & SU_FITZ_WEAPON2) cl.stats[STAT_WEAPON] |= MSG_ReadByte() << 8;
    if (bits & SU_FITZ_ARMOR2) cl.stats[STAT_ARMOR] |= MSG_ReadByte() << 8;
    if (bits & SU_FITZ_AMMO2) cl.stats[STAT_AMMO] |= MSG_ReadByte() << 8;
    if (bits & SU_FITZ_SHELLS2) cl.stats[STAT_SHELLS] |= MSG_ReadByte() << 8;
    if (bits & SU_FITZ_NAILS2) cl.stats[STAT_NAILS] |= MSG_ReadByte() << 8;
    if (bits & SU_FITZ_ROCKETS2) cl.stats[STAT_ROCKETS] |= MSG_ReadByte() << 8;
    if (bits & SU_FITZ_CELLS2) cl.stats[STAT_CELLS] |= MSG_ReadByte() << 8;
    if (bits & SU_FITZ_WEAPONFRAME2) cl.stats[STAT_WEAPONFRAME] |= MSG_ReadByte() << 8;
    if (bits & SU_FITZ_WEAPONALPHA) MSG_ReadByte();  // FIXME - TODO
}
...
void CL_NewTranslation(int slot) {
    ...
    if (slot > cl.maxclients) Sys_Error("%s: slot > cl.maxclients", __func__);
...
=====================
*/
void CL_ParseStatic(unsigned int bits) {
    entity_t *ent;
    int i;
...
/* FITZ protocol */
static void CL_ParseFitzStaticSound2(void) {
    vec3_t org;
    int sound_num, vol, atten;
    int i;
...
static void SHOWNET(const char *msg) {
...
void CL_ParseServerMessage(void) {
    ...
    while (1) {
        if (msg_badread) Host_Error("%s: Bad server message", __func__);
        ...
        switch (cmd) {
            ...
            case svc_version:
                ...
                if (!Protocol_Known(version))
                    Host_Error("%s: Server returned unknown protocol version %i", __func__,
                               version);
                cl.protocol = version;
                break;
            ...
            case svc_lightstyle:
                ...
                if (stylenum >= MAX_LIGHTSTYLES) Sys_Error("svc_lightstyle > MAX_LIGHTSTYLES");
            ...
            case svc_updatecolors:
                ...
                if (playernum == cl.viewentity) cl_color.value = colors;
            ...
            default:
                Host_Error("%s: Illegible server message. Previous was %s", __func__,
                           svc_strings[prevcmd]);
        }
        prevcmd = cmd;
    }
