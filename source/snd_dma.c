...
void S_Init(void) {
    ...
    if (sound_started) Con_Printf("Sound sampling rate: %i\n", shm->speed);
...
static sfx_t *S_FindName(const char *name) {
    ...
    if (!name) Sys_Error("%s: NULL", __func__);
    if (strlen(name) >= MAX_QPATH) Sys_Error("%s: name too long: %s", __func__, name);

    /* see if already loaded */
    for (i = 0; i < num_sfx; i++)
        if (!strcmp(known_sfx[i].name, name)) return &known_sfx[i];

    if (num_sfx == MAX_SFX) Sys_Error("%s: out of sfx_t", __func__);
...
static channel_t *SND_PickChannel(int entnum, int entchannel) {
...
#ifdef NQ_HACK
        if (channel->entnum == cl.viewentity && entnum != cl.viewentity && channel->sfx) continue;
#endif
#ifdef QW_HACK
    ...
    if (first_to_die && first_to_die->sfx) first_to_die->sfx = NULL;
...
void S_StartSound(int entnum, int entchannel, sfx_t *sfx, vec3_t origin, float fvol,
                  float attenuation) {
