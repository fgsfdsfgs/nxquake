...
qsocket_t *NET_NewQSocket(void) {
    ...
    if (!net_freeSockets) return NULL;

    if (net_activeconnections >= svs.maxclients) return NULL;
...
void NET_FreeQSocket(qsocket_t *sock) {
    ...
    if (sock == net_activeSockets)
        ...
        if (!s) Sys_Error("%s: not active", __func__);
    }
...
static void Slist_Send(void) {
    ...
    for (i = 0; i < net_numdrivers; i++) {
        ...
        if (!slistLocal && IS_LOOP_DRIVER(net_driver)) continue;
        if (net_driver->initialized == false) continue;
...
void NET_Init(void) {
    ...
    for (i = 0; i < net_numdrivers; i++) {
        ...
        if (listening) net_driver->Listen(true);
    }

    if (num_inited == 0 && cls.state == ca_dedicated) Sys_Error("Network not available!");
