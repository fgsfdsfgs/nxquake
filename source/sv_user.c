...
void SV_SetIdealPitch(edict_t *player) {
    ...
    if (!((int)player->v.flags & FL_ONGROUND)) return;
    ...
    for (i = 0; i < MAX_FORWARD; i++) {
        ...
        if (trace.allsolid) return;  // looking at a wall, leave ideal the way is was

        if (trace.fraction == 1) return;  // near a dropoff
...
==================
*/
static void SV_UserFriction(edict_t *player) {
...
static void SV_Accelerate(const vec3_t wishdir, const vec_t wishspeed, vec3_t velocity) {
    int i;
...
static void SV_AirAccelerate(vec3_t wishveloc, const vec_t wishspeed, vec3_t velocity) {
    int i;
...
static void DropPunchAngle(vec3_t punchangle) {
    vec_t length;
...
===================
*/
static void SV_WaterMove(const usercmd_t *cmd, edict_t *player) {
    int i;
...
static void SV_WaterJump(edict_t *player) {
...
===================
*/
static void SV_AirMove(const usercmd_t *cmd, edict_t *player) {
...
===================
*/
static void SV_ClientThink(client_t *client) {
...
===================
*/
static void SV_ReadClientMove(client_t *client, usercmd_t *move) {
    edict_t *player = client->edict;
    ...
    if (impulse) player->v.impulse = impulse;
}
...
======================
*/
static void SV_Name_f(client_t *client) {
    char new_name[16];
    const char *arg;
    ...
    if (!strcmp(client->name, new_name)) return;
...
==================
*/
static void SV_Color_f(client_t *client) {
    int top, bottom;
...
==================
*/
static void SV_Status_f(client_t *client) {
    client_t *other;
    int seconds;
    int minutes;
    int hours = 0;
    ...
    if (tcpipAvailable) SV_ClientPrintf(client, "tcp/ip:  %s\n", my_tcpip_address);
...
==================
*/
static void SV_God_f(client_t *client) {
    edict_t *player;
...
==================
*/
static void SV_Fly_f(client_t *client) {
    edict_t *player;
...
==================
*/
static void SV_Noclip_f(client_t *client) {
    edict_t *player;
...
==================
*/
static void SV_Notarget_f(client_t *client) {
    edict_t *player;
...
==================
*/
static void SV_Ping_f(client_t *client) {
    int i, j;
    float total;
    client_t *other;
    ...
    for (i = 0; i < svs.maxclients; i++, other++) {
        if (!other->active) continue;
        total = 0;
        for (j = 0; j < NUM_PING_TIMES; j++) total += other->ping_times[j];
        total /= NUM_PING_TIMES;
        SV_ClientPrintf(client, "%4i %s\n", (int)(total * 1000), other->name);
    }
}

static qboolean SV_Give_Hipnotic(edict_t *player, char item, char extra) {
    switch (item) {
        case '0':
...
static qboolean SV_Give_Rogue(edict_t *player, char item, int amount) {
    eval_t *ammofield;
    const char *ammo;
    ...
    switch (item) {
        ...
        case 'n':
            ...
            break;
        case 'l':
            ammo = "ammo_lava_nails";
            break;
        case 'r':
            ammo = "ammo_rockets1";
            break;
        case 'm':
            ammo = "ammo_multi_rockets";
            break;
        case 'c':
            ammo = "ammo_cells1";
            break;
    ...
    if (ammofield) {
        ...
        switch (item) {
            case 'n':
                if (player->v.weapon <= IT_LIGHTNING) player->v.ammo_nails = amount;
                break;
            case 'l':
                if (player->v.weapon > IT_LIGHTNING) player->v.ammo_nails = amount;
                break;
            case 'r':
                if (player->v.weapon <= IT_LIGHTNING) player->v.ammo_rockets = amount;
                break;
            case 'm':
                if (player->v.weapon > IT_LIGHTNING) player->v.ammo_rockets = amount;
                break;
            case 'c':
                if (player->v.weapon <= IT_LIGHTNING) player->v.ammo_cells = amount;
                break;
            case 'p':
                if (player->v.weapon > IT_LIGHTNING) player->v.ammo_cells = amount;
                break;
...
==================
*/
static void SV_Give_f(client_t *client) {
    edict_t *player;
    char item, extra;
    int amount;
    ...
    if (hipnotic && SV_Give_Hipnotic(player, item, extra)) return;
    if (rogue && SV_Give_Rogue(player, item, amount)) return;
...
==================
*/
static void SV_Kill_f(client_t *client) {
    edict_t *player = client->edict;
...
==================
*/
static void SV_Pause_f(client_t *client) {
    edict_t *player;
...
==================
*/
static void SV_PreSpawn_f(client_t *client) {
    if (client->spawned) {
...
==================
*/
static void SV_Spawn_f(client_t *client) {
    client_t *other;
    edict_t *player;
    int i;
    ...
    for (i = 0; i < 2; i++) MSG_WriteAngle(&client->message, player->v.angles[i]);
...
==================
*/
static void SV_Begin_f(client_t *client) { client->spawned = true; }
...
==================
*/
static void SV_Kick_f(client_t *client) {
    ...
    if (Cmd_Argc() > 2 && strcmp(Cmd_Argv(1), "#") == 0) {
        ...
        if (clientnum < 0 || clientnum >= svs.maxclients) return;
        ...
        for (i = 0; i < svs.maxclients; i++, victim++) {
            ...
            if (!strcasecmp(victim->name, Cmd_Argv(1))) break;
        }
        if (i == svs.maxclients) return;
    }
...
static void SV_Say(client_t *client, qboolean teamonly) {
    client_t *recipient;
    int i;
    size_t len, space;
    const char *msg;
    ...
    for (i = 0; i < svs.maxclients; i++, recipient++) {
        if (!recipient->active || !recipient->spawned) continue;
        if (teamplay.value && teamonly && client->edict->v.team != recipient->edict->v.team)
            continue;
...
static void SV_Say_f(client_t *client) { SV_Say(client, false); }

static void SV_Say_Team_f(client_t *client) { SV_Say(client, true); }
...
static void SV_Tell_f(client_t *client) {
    ...
    for (i = 0; i < svs.maxclients; i++, recipient++) {
        if (!recipient->active || !recipient->spawned) continue;
        if (strcasecmp(recipient->name, Cmd_Argv(1))) continue;
        SV_ClientPrintf(recipient, "%s", text);
        break;
    }
}
...
static client_command_t client_commands[] = {
    {"name", SV_Name_f},         {"color", SV_Color_f},
    {"status", SV_Status_f},     {"say", SV_Say_f},
    {"say_team", SV_Say_Team_f}, {"tell", SV_Tell_f},
    {"god", SV_God_f},           {"fly", SV_Fly_f},
    {"noclip", SV_Noclip_f},     {"notarget", SV_Notarget_f},
    {"give", SV_Give_f},         {"ping", SV_Ping_f},
    {"kill", SV_Kill_f},         {"pause", SV_Pause_f},
    {"kick", SV_Kick_f},         {"ban", NET_Ban_f},
    {"prespawn", SV_PreSpawn_f}, {"spawn", SV_Spawn_f},
    {"begin", SV_Begin_f},       {NULL, NULL},
};

static void SV_ExecuteClientCommand(const char *command_string, client_t *client) {
...
===================
*/
static qboolean SV_ReadClientMessage(client_t *client) {
    const char *message;
    int ret, cmd;
    ...
    nextmsg:
        ...
        while (1) {
            if (!client->active) return false;  // a command caused an error
...
void SV_RunClients(void) {
    ...
    for (i = 0; i < svs.maxclients; i++, client++) {
        ...
        if (!sv.paused && (svs.maxclients > 1 || key_dest == key_game)) SV_ClientThink(client);
    }
}
