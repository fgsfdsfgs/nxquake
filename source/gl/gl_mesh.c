...
================
*/
static int StripLength(aliashdr_t *hdr, int starttri, int startv, const mtriangle_t *tris) {
...
nexttri:
    for (j = starttri + 1, check = &tris[starttri + 1]; j < hdr->numtris; j++, check++) {
...
===========
*/
static int FanLength(aliashdr_t *hdr, int starttri, int startv, const mtriangle_t *tris) {
    const mtriangle_t *last, *check;
    int m1, m2;
    int j;
    int k;
...
================
*/
static void BuildTris(aliashdr_t *hdr, const mtriangle_t *tris, const stvert_t *stverts) {
    int i, j, k;
    int startv;
    float s, t;
...
static void GL_MeshSwapCommands(void) {
    int i;
 ...
 */
static qboolean GL_MeshVerifyCommands(const aliashdr_t *hdr, const model_t *model) {
    int i, length, verts;
...
================
*/
void GL_LoadMeshData(const model_t *model, aliashdr_t *hdr, const alias_meshdata_t *meshdata,
