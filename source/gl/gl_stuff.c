...
void QGL_ReloadShaders( void )
{
  ...
  for (GLuint p = 0; p < 9; ++p) u_mvp[p] = glGetUniformLocation( programs[p], "uMVP" );
...
// HACKHACKHACK: GL function wrappers to go with this shit and a whole GL1 IMM emulator
// because fuck you mesa
...
void QGL_Deinit(void)
{
    ...
    QGL_FreeShaders();
    free(imm_vertbuf); imm_vertp = imm_vertbuf = NULL;
}

void qglBegin( GLenum prim )
{
    if (!imm_vertbuf) Sys_Error("what the hell");
