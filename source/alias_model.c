...
=================
*/
static void *Mod_LoadAliasFrame(aliashdr_t *aliashdr, void *buffer, maliasframedesc_t *frame,
...
=================
*/
static void *Mod_LoadAliasFrameGroup(aliashdr_t *aliashdr, void *buffer, maliasframedesc_t *frame,
                                     alias_posedata_t *posedata) {
    ...
    for (i = 0; i < frame->numposes; i++) {
        ...
        *interval = LittleFloat(group->intervals[i].interval);
        if (*interval++ <= 0) Sys_Error("%s: interval <= 0", __func__);
        posedata->numposes++;
...
=================
*/
static void *Mod_LoadAliasFrames(aliashdr_t *aliashdr, void *buffer, alias_posedata_t *posedata) {
    const int numframes = aliashdr->numframes;
    maliasframedesc_t *frame;
    int i;

    if (numframes < 1) Sys_Error("%s: Invalid # of frames: %d", __func__, numframes);
...
=================
*/
static void *Mod_LoadAliasSkin(aliashdr_t *aliashdr, void *buffer, maliasskindesc_t *skin,
...
=================
*/
static void *Mod_LoadAliasSkinGroup(aliashdr_t *aliashdr, void *buffer, maliasskindesc_t *skin,
                                    alias_skindata_t *skindata) {
...
===============
*/
static void *Mod_LoadAliasSkins(aliashdr_t *aliashdr, const model_loader_t *loader, model_t *model,
                                void *buffer, alias_skindata_t *skindata) {
    maliasskindesc_t *skin;
    int i;

    if (aliashdr->numskins < 1)
        Sys_Error("%s: Invalid # of skins: %d", __func__, aliashdr->numskins);
    if (aliashdr->skinwidth & 0x03) Sys_Error("%s: skinwidth not multiple of 4", __func__);
...
static void *Mod_LoadAliasSTVerts(const aliashdr_t *aliashdr, void *buffer,
...
static void *Mod_LoadAliasTriangles(const aliashdr_t *aliashdr, void *buffer,
...
static void Mod_AliasCRC(const model_t *model, const byte *buffer, int bufferlen) {
#ifdef QW_HACK
    unsigned short crc;
    const char *crcmodel = NULL;

    if (!strcmp(model->name, "progs/player.mdl")) crcmodel = "pmodel";
    if (!strcmp(model->name, "progs/eyes.mdl")) crcmodel = "emodel";

    if (crcmodel) {
        crc = CRC_Block(buffer, bufferlen);
        Info_SetValueForKey(cls.userinfo, crcmodel, va("%d", (int)crc), MAX_INFO_STRING);
        if (cls.state >= ca_connected) {
            MSG_WriteByte(&cls.netchan.message, clc_stringcmd);
            MSG_WriteStringf(&cls.netchan.message, "setinfo %s %d", crcmodel, (int)crc);
        }
    }
#endif
}
 ...
 */
void Mod_AliasLoaderAlloc(const mdl_t *mdl, alias_meshdata_t *meshdata, alias_posedata_t *posedata,
                          alias_skindata_t *skindata) {
    const void *buffer;
...
void Mod_LoadAliasModel(const model_loader_t *loader, model_t *model, void *buffer) {
    ...
    if (version != ALIAS_VERSION)
        Sys_Error("%s has wrong version number (%i should be %i)", model->name, version,
                  ALIAS_VERSION);
    ...
    if (aliashdr->skinheight > MAX_LBM_HEIGHT)
        Sys_Error("model %s has a skin taller than %d", model->name, MAX_LBM_HEIGHT);
    if (aliashdr->numverts <= 0) Sys_Error("model %s has no vertices", model->name);
    if (aliashdr->numverts > MAXALIASVERTS)
        Sys_Error("model %s has too many vertices", model->name);
    if (aliashdr->numtris <= 0) Sys_Error("model %s has no triangles", model->name);
...
#define MAX_MCACHE 512 /* TODO: cvar controlled */
    ...
    for (i = 0; i < MAX_MCACHE - 1; i++, model++) model->next = model + 1;
    model->next = NULL;
}
...
void Mod_ClearAlias(void) {
     ...
     */
    for (model = mcache.overflow.next; model; model = model->next)
        if (model->cache.data) Cache_Free(&model->cache);
    mcache.overflow.next = NULL;
}

const model_t *Mod_AliasCache(void) { return mcache.used.next; }

const model_t *Mod_AliasOverflow(void) { return mcache.overflow.next; }
