...
int R_SpriteDataSize(int pixels) { return pixels * r_pixbytes; }
...
void R_SpriteDataStore(mspriteframe_t *frame, const char *modelname, int framenum, byte *pixels) {
    ...
    if (r_pixbytes == 1) {
        ...
        for (i = 0; i < size; i++) pixout[i] = d_8to16table[pixels[i]];
    } else {
...
static void R_SetupAndDrawSprite() {
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (nump < 3) return;
        if (nump >= MAXWORKINGVERTS) Sys_Error("%s: too many points", __func__);
    }
