...
#define STREE_ROOT \
    (struct stree_root) { 0, 0, -1, QRB_ROOT, NULL }
#define DECLARE_STREE_ROOT(_x) \
    struct stree_root _x = {   \
        .entries = 0, .maxlen = 0, .minlen = -1, .root = QRB_ROOT, .stack = NULL}
 ...
 */
#define STree_ForEach_After(root__, node__, str__)                           \
    for (STree_ForEach_Init__((root__), &(node__)), ({                       \
             if (str__) STree_ForEach_After__((root__), &(node__), (str__)); \
         });                                                                 \
         STree_WalkLeft__((root__), &(node__)); STree_WalkRight__(&(node__)))
#define STree_ForEach(root__, node__) STree_ForEach_After(root__, node__, NULL)
#define STree_ForEach_break(root__)      \
    ({                                   \
        STree_ForEach_Cleanup__(root__); \
        break;                           \
    })
